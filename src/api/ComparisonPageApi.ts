...
export async function fetchAllPredictionParams(userId: number) {
    let predictionParams, userSpecificParams;
    ...
    if (result[1].status === 'fulfilled') {
        userSpecificParams = result[1].value;
    }
    return { predictionParams, userSpecificParams };
}
...
function fetchEnergyUsage(userId: number): Promise<UserSpecificParams> {
